...
std::string ArchRepo::httpGet(const std::string &url) const {
    std::string cmd = "curl -sL --proto =https --connect-timeout 15 --max-time 15 '" + url + "' 2>/dev/null";
...
ArchPkgInfo ArchRepo::search(const std::string &rawName) {
    ...
    if (info.filename.empty()) {
        ...
        info.filename = info.pkgname + "-" + info.version + "-" + arch + ".pkg.tar.zst";
    }
...
bool ArchRepo::downloadAndExtract(const std::string &pkgname, const std::string &destdir, const std::string &cachedir) {
    ...
    std::string dlDir = cachedir.empty() ? (destdir + "/.cache") : cachedir;
    std::string mkdir = "mkdir -p '" + dlDir + "' && mkdir -p '" + destdir + "'";
    std::system(mkdir.c_str());

    std::string url = std::string(ARCHIVE_URL) + info.filename;
    std::string localFile = dlDir + "/" + info.filename;

    std::string checkCmd = "test -f '" + localFile + "'";
    bool cached = (std::system(checkCmd.c_str()) == 0);

    if (!cached) {
        std::cout << "    [arch] downloading " << info.filename
                  << " (" << info.repo << "/" << info.pkgname << ")\n";
        std::string dl = "curl -fL --proto =https --connect-timeout 30 --max-time 120 --progress-bar -o '"
                         + localFile + "' '" + url + "' 2>&1";
        ...
        std::cout << "    [arch] using cached " << info.filename << "\n";
    }
    ...
    std::string fixLinks =
        "find '" + destdir + "' -type l -lname '/*' 2>/dev/null | while IFS= read -r link; do\n"
        "  target=$(readlink \"$link\")\n"
        "  reldest='" + destdir + "'\"$target\"\n"
        "  if [ -e \"$reldest\" ] || [ -L \"$reldest\" ]; then\n"
        "    linkdir=$(dirname \"$link\")\n"
        "    rel=$(realpath --relative-to=\"$linkdir\" \"$reldest\" 2>/dev/null)\n"
        "    [ -n \"$rel\" ] && ln -sf \"$rel\" \"$link\"\n"
        "  fi\n"
        "done";
    std::system(fixLinks.c_str());

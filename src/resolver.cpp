...
void Resolver::resolveOne(const std::string &pkgname, std::vector<std::string> &order) {
    ...
    if (archInfo.found) {
        ...
        if (pkgname == "go") {
            // Go cgo needs pkg-config for library discovery
            ArchRepo subRepo;
            for (auto &tool : {"pkg-config"}) {
                if (systemDeps_.find(tool) != systemDeps_.end()) continue;
                auto subInfo = subRepo.search(tool);
                if (subInfo.found) {
                    std::cout << "  [resolver] implicit toolchain: \"" << tool
                              << "\" (needed by Go cgo)\n";
                    systemDeps_.insert(tool);
                    cachedInfo_[tool] = subInfo;
                }
            }
        }
        return;

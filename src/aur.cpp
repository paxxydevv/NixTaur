...
std::string Aur::httpGet(const std::string &url) const {
    ...
    std::string cmd = "curl -sL --connect-timeout 30 --max-time 15 '" + url + "' 2>&1";
...
bool Aur::downloadSource(const std::string &pkgname) {
    ...
    std::string cmd = "curl -sL '" + url + "' 2>/dev/null | tar -xz 2>/dev/null";
...
std::vector<std::string> Aur::checkUpgrades(const std::vector<std::string> &installed) {
    (void)installed;
    return {};
}
